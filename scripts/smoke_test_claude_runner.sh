...
image_name="${CLAUDE_IMAGE_NAME:-myqqbot/claude-runner:local}"
...
"$uv_bin" run python -m http.server "$api_proxy_port" --bind 0.0.0.0 --directory "$tmp_root/api_proxy" >/tmp/myqqbot-api-proxy.log 2>&1 &
api_proxy_pid=$!
"$uv_bin" run python -m http.server "$blocked_host_port" --bind 0.0.0.0 --directory "$tmp_root/blocked" >/tmp/myqqbot-blocked-host.log 2>&1 &
blocked_pid=$!
sleep 1

docker build -t "$image_name" -f "$repo_root/docker/claude-runner/Dockerfile" "$repo_root" >/tmp/myqqbot-docker-build.log
run_args=(
  -d
  --name "$container_name"
  --read-only
...
if ! docker run "${run_args[@]}" --storage-opt size=50G "$image_name" >/tmp/myqqbot-docker-run.log 2>/tmp/myqqbot-docker-run.err; then
  if grep -q "storage-opt is supported only" /tmp/myqqbot-docker-run.err; then
    echo "storage-opt size=50G unsupported on this host; retrying smoke test without disk quota enforcement."
    docker run "${run_args[@]}" "$image_name" >/tmp/myqqbot-docker-run.log
  else
    cat /tmp/myqqbot-docker-run.err >&2
    exit 1
  fi
fi

docker exec "$container_name" claude --version >/tmp/myqqbot-claude-version.log
