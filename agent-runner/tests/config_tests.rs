...
fn settings_use_expected_defaults() {
    ...
    assert_eq!(settings.anthropic_base_url, "http://host.docker.internal:9000");
...
fn settings_allow_runner_limit_overrides() {
    let env = std::collections::HashMap::from([
        ("CLAUDE_IMAGE_NAME".to_string(), "custom/claude:1".to_string()),
        ...
        ("SESSION_DB_PATH".to_string(), "/tmp/state/runner.db".to_string()),
    ]);
